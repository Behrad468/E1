...
T_top = 24.44 + 273.15;
T_bottom = 34.22 + 273.15;
depth = 1524;
number_grids = ceil (depth/dz) ;
U_to_top = 105.2;  % W/m2 C
U_to_end = 105.2;
Cp_W = 4.18; 
Cp_O = 2; 
Cp_G = 1; 
...
iteration_number = 1;
for time = 0: dt: 84600*dt % 10 days
    if time == 0 && iteration_number ==1   % initial condition
        for i = 1: number_grids   
            initial_T (i,1) = reservoir_temp (T_top, T_bottom, i) + 3; %#ok
            initial_P (i,1) = (0.6*101325/(0.3048*14.7)) * i * dz; %#ok
            initial_alpha_gas (i,1) = 0.4; %#ok
            initial_alpha_water (i,1) = 0.1; %#ok
    ...
    while true        
        ...
        update = SOR_method (Jacobian,-answer,0.5, initial_guess, 10^-4);
...
Re = (rho_mixture*V_mixture*0.0762)/(0.001*alpha_w+alpha_o*0.01);
f_tp = 24/ Re;
end
